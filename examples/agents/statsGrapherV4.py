...
if __name__ == '__main__':
    ...
    weak_curr = [
        "/home/alexander/Documents/dev/data_recording_dir/100x_raw_data/weakstrong_x10_currweakest/"
    ]
    ...
    success_percents = []
    success_time_avgs = []
    histogram_data_file = open("/home/alexander/Documents/dev/data_recording_dir/100x_raw_data/max_cloth_histogram_data.txt", mode='w')
    for prefixes in prefix_list:
        ...
        for prix,prefix in enumerate(prefixes):
            ...
            filename = "progress_history1.txt"
            ...
            if max_value_histogram:
                #print maximums of each data stream and print to console
                histogram_data_file.write(prefix)
                histogram_data_file.write("\n")
                histogram_data_file.write(str(titles))
                histogram_data_file.write("\n")
                data_maximums = []
                for r in range(len(data)):
                    data_maximums.append([])
                    for c in range(len(data[r])):
                        data_maximums[-1].append([])
                        for s in range(len(data[r][c])):
                            max = int(np.amax(np.array(data[r][c][s])))
                            data_maximums[-1][-1].append(max)
                    #print(data_maximums[-1])
                    histogram_data_file.write(str(data_maximums[-1]))
                    histogram_data_file.write("\n")
            else:
