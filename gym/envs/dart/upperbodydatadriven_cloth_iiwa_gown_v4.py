...
class DartClothUpperBodyDataDrivenClothIiwaGownEnvV4(DartClothUpperBodyDataDrivenClothBaseEnv, utils.EzPickle):
    def __init__(self):
        ...
        self.elbowLimitsVarObs = True #if true, modify limits of the elbow joint
    ...
    def additionalResets(self):
        ...
        if(self.trackPosePath):
            ...
            while (not good):
                ...
                if not good:
                    p0 = self.iiwa_skel.bodynodes[3].to_world(np.zeros(3)) + pyutils.sampleDirections(num=1)[
                                                                                 0] * diskRad
    ...
    def extraRenderFunction(self):
        ...
        #draw the new sampling distribution
        if False:
            diskRad = self.robotPathParams['p0_disk_rad']
            org = self.iiwa_skel.bodynodes[3].to_world(np.zeros(3))
            org[0] -= self.robotPathParams['p0_depth_offset']
            org[2] -= self.robotPathParams['p0_depth_offset']
            pXY = Plane(org=org, normal=np.array([0,0,1]))
            pZY = Plane(org=org, normal=np.array([1,0,0]))
            pXY.draw()
            pZY.draw()
            renderUtils.drawSphere(pos=org, rad=diskRad, solid=False)
