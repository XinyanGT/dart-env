...
class DartOP3Env(dart_env.DartEnv, utils.EzPickle):
    ...
    def __init__(self):
        ...
        dart_env.DartEnv.__init__(self, ['robotis_op3/ground1.urdf', 'robotis_op3/robotis_op3.urdf'], int(self.control_interval / self.sim_timestep), obs_dim,
        ...
        self.init_position_x = self.robot_skeleton.bodynode('body_link').C[0]
    ...
    def adjust_root(self): # adjust root dof such that foot is roughly flat
        q = self.robot_skeleton.q
        q[1] += -1.57 - np.array(euler_from_matrix(self.robot_skeleton.bodynode('l_ank_roll_link').T[0:3, 0:3], 'sxyz'))[1]
    ...
    def get_sim_bno55(self):
        ...
        tinv = np.linalg.inv(self.robot_skeleton.bodynode('body_link').T[0:3, 0:3])
        angvel = self.robot_skeleton.bodynode('body_link').com_spatial_velocity()[0:3]
        langvel = np.dot(tinv, angvel)

        euler = np.array(euler_from_matrix(self.robot_skeleton.bodynode('body_link').T[0:3, 0:3], 'sxyz'))
        ...
        for i in range(self.frame_skip):
            ...
            if self.task_mode == self.BONGOBOARD:
                if self.t < self.assist_timeout:
                    force = self._bodynode_spd(self.robot_skeleton.bodynode('body_link'), self.current_assist[0], 1)
                    self.robot_skeleton.bodynode('body_link').add_ext_force(np.array([0, force, 0]))

                    force = self._bodynode_spd(self.robot_skeleton.bodynode('body_link'), self.current_assist[1], 0)
                    self.robot_skeleton.bodynode('body_link').add_ext_force(np.array([force, 0, 0]))
    ...
    def step(self, a):
        ...
        upright_rew = np.abs(euler_from_matrix(self.robot_skeleton.bodynode('body_link').T[0:3, 0:3], 'sxyz')[1])
        ...
        if self.task_mode == self.WALK and 0.15 < np.linalg.norm(self.robot_skeleton.bodynode('r_ank_roll_link').C - self.robot_skeleton.bodynode('l_ank_roll_link').C):
            done = True
